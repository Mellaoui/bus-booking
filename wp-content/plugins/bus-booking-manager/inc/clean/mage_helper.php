...
function mage_qty_box($price,$name, $return) {
    ...
    if (mage_available_seat(wbbm_convert_date_to_php($date)) > 0) {
        ...
        <div class="mage_form_group">
            ...
            <input type="text"
                   ...
                   min="0"
                   max="<?php echo mage_available_seat(wbbm_convert_date_to_php($date)); ?>"
                   required
            />
...
function mage_route_list($drop = false) {
    ...
    echo '<div class="mage_input_select_list"'.($drop ? "id=wbtm_dropping_point_list" : "").' ><ul>';
